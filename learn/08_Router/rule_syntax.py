...
这个对象代表了正则表达式在 rule 字符串中找到的一个成功匹配实例.
它不仅包含了匹配到的字符串, 还包含了匹配发生的位置信息以及正则表达式中括号定义的捕获组信息.
...
findall 会直接返回一个包含所有捕获组字符串的列表, 它丢失了位置信息(start/end).
在写 Router 时, 我们需要位置信息来切分出通配符之间的"静态部分"(prefix), 所以 finditer 是唯一的正确选择.

捕获组的编号:
在使用复杂的正则表达式(如 rule_syntax)时, 捕获组很多. 在 _itertokens 方法中, 代码使用了 g = match.groups() 后通过切片 g[4:7] 访问,这虽然高效, 但对维护者不友好.
在更现代的 Python 代码中, 我们会倾向于使用 命名捕获组 (?P<name>...), 然后通过 match.group('name') 来访问, 这样代码的可读性会更高.
"""
