...
class TestDateParser(unittest.TestCase):
    ...
    def test_rfc850(self):
        ...
        ts = time.time()
        t = time.gmtime(ts)
        rs = time.strftime("%%s, %d-%%s-%y %H:%M:%S GMT", t) % (
            weekday_full[t.tm_wday],
            month_abbr[t.tm_mon],
        )
        self.assertEqual(int(ts), int(parse_date(rs)))
    ...
    def test_asctime(self):
        ...
        ts = time.time()
        t = time.gmtime(ts)
        rs = time.strftime("%%s %%s %d %H:%M:%S %Y", t) % (
            weekday_abbr[t.tm_wday],
            month_abbr[t.tm_mon],
        )
        self.assertEqual(int(ts), int(parse_date(rs)))
...
class TestSendFile(unittest.TestCase):
    ...
    def test_mime(self):
        ...
        self.assertTrue(
            ...
            in ("application/x-python-code", "text/x-python")
        )
    ...
    def test_ims(self):
        ...
        self.assertEqual(
            int(os.stat(__file__).st_mtime), parse_date(res.headers["Last-Modified"])
        )
        ...
        self.assertEqual(
            open(__file__, "rb").read(), static_file(basename, root=root).body.read()
        )
    ...
    def test_download(self):
        ...
        self.assertEqual(
            'attachment; filename="%s"' % basename, f.headers["Content-Disposition"]
        )
        request.environ["HTTP_IF_MODIFIED_SINCE"] = bottle.http_date(100)
    ...
    def test_range(self):
        ...
        self.assertEqual(
            "bytes 10-25/%d" % len(open(__file__, "rb").read()),
