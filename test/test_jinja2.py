...
class TestJinja2Template(unittest.TestCase):
    ...
    def test_file(self):
        ...
        with chdir(__file__):
            t = Jinja2Template(name="./views/jinja2_simple.tpl", lookup=["."]).render(
                var="var"
            )
            self.assertEqual("start var end", "".join(t))
    ...
    def test_name(self):
        ...
        with chdir(__file__):
            t = Jinja2Template(name="jinja2_simple", lookup=["./views/"]).render(
                var="var"
            )
            self.assertEqual("start var end", "".join(t))
    ...
    def test_custom_filters(self):
        ...
        settings = dict(
            filters={"star": lambda var: touni("").join((touni("*"), var, touni("*")))}
        )
